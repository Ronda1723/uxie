...
export async function startRecording(meetingId: number): Promise<void> {
  try {
    // Engine spawns the UxieAudioTap.app sub-bundle (mic + system audio
    // via AVCaptureSession + ScreenCaptureKit) and opens a long-form
    // Deepgram socket. The renderer's mic capture path is NOT used for
    // meeting recording — the Swift sidecar owns audio capture so we
    // also pick up other participants on Zoom / Meet / Teams.
